...
@section('content')
    <div class="p-5">
        <div class="flex justify-between items-center">
            <h1 class="text-gray-800 pl-2 w-full border-l-4 border-[#5A88C6] text-2xl mb-5">Admins</h1>
                ...
                <div class="col-span-1 bg-white rounded-lg shadow hover:shadow-lg transition">
                    <div class="pt-4 pb-2 mx-auto w-fit mb-5">
                        <div class="flex items-center">
                            <div class="shrink-0">
                                <img src="{{ $admin->avatar ? asset('storage/'.$admin->avatar) : asset('images/user-picture.jpg') }}" class="rounded-full w-14" alt="Avatar">
                            </div>
                            <div class="grow ml-3">
                                <p class="text-sm font-semibold">{{ $admin->name }}</p>
                                <p class="text-xs text-gray-500">{{ $admin->nip }}</p>
                                <span class="p-1 rounded-md {{ $admin->useronopd?->valid ? 'bg-green-400' : 'bg-red-400' }} text-white text-xs">validasi: {{ $admin->useronopd?->valid ? 'Valid' : 'Belum Valid' }}</span>
                            </div>
                        </div>
                    </div>
                    <hr class="my-2">
                    <div class="w-full flex divide-x-2 text-xs">
                        <div class="grid place-items-center w-full">OPD: {{ $admin->useronopd?->opd->opd_name }}</div>
                        <div class="grid place-items-center w-full">Ruang: - </div>
                    </div>
                    <hr class="mt-2">
                    <div class="transition hover:bg-blue-50 text-blue-400 font-semibold">
                        <a href="/admin/kepegawaian/admins/{{ $admin->email }}/edit" class="inline-block w-full h-full p-2 text-center">Detail</a>
                    </div>
                </div>
            @empty
        ...
        <div class="my-2">
            {{ $admins->withQueryString()->links('vendor.pagination.tailwind') }}
        </div>
